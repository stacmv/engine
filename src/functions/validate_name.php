<?php
function validate_rule_fio($key, $value, $rule_params, ChangesSet $changes){
    $res = true;
    $aFio = explode(" ",$value);
    foreach($aFio as $name){
        $res = $res && (validate_field_type_name($key, $name, $changes) == "");
    };
...
function validate_field_type_name($key, $value, ChangesSet $changes){
