...
function db_get_meta($db_table, $attr){
    ...
    if ($tables){
        if (!isset($tables[0])){ // there is only one table defined for db in xml
            $tables = array($tables);
        };
...
function db_edit($db_table, $id, ChangesSet $changes, $comment=""){
    ...
    $conflicts_ignore = db_get_meta($db_table,"conflicts_ignore"); // список полей, конфликты в которых надо игнорировать
    if ($conflicts_ignore){
        $conflicts_ignore = array_map("trim", explode(",", $conflicts_ignore));
    };
    foreach ($changes->to as $key=>$value){
        ...
        if (isset($changes->from[$key])){
            ...
            if (strcmp((string) $form_from, (string) $stored_in_db) !=0 ){
                ...
                if (is_string($changes->from[$key]) && is_string($object[$key]) && (preg_replace('~\R~u', "\n", $changes->from[$key]) == preg_replace('~\R~u', "\n", $object[$key])) ){
                    ...
                    // случай null и 0
                }elseif(!empty($conflicts_ignore) && in_array($key, $conflicts_ignore)) {
                    // Это не конфликт.
                }else{
...
function db_sqlite_register_function($dbh, $func_name){
    ...
    case "lower":
        if (method_exists($dbh, "sqliteCreateFunction")){ // this is experimental method since PHP 5.1 (http://php.net/manual/ru/pdo.sqlitecreatefunction.php)
          return $dbh->sqliteCreateFunction("lower", function($value){
              $value = (string) $value; // maybe NULL in db
